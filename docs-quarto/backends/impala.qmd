...
```{python}
from _utils import badges, render_doc

print(
    badges(imports = [], exports = [])
)
```
...
# Note that the original code also included exists_database, but this is not
# on the Backend, so gets silently dropped from documentation.
print(
    render_doc(
